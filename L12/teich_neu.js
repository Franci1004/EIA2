...
(function (L12_Ententeich) {
    ...
    class Brotkrümel {
        position;
        constructor(x, y) {
            this.position = new L12_Ententeich.Vector(x, y);
        }
        draw(crc2) {
    ...
    function createDucks() {
        for (let i = 0; i < 3; i++) {
            ...
            const duck = new Ente(new L12_Ententeich.Vector(20, 20), 'yellow', canvas, crc2, new L12_Ententeich.Vector(startX, startY));
            ducks.push(duck);
        }
    }
    ...
    function createBabyDucks(parentDuck) {
        for (let i = 0; i < 3; i++) {
            const babyDuck = new Ente(new L12_Ententeich.Vector(10, 10), 'yellow', canvas, crc2, new L12_Ententeich.Vector(parentDuck.position.x, parentDuck.position.y));
    ...
    class Background {
        ...
        constructor(canvas) {
            ...
            for (let i = 0; i < 6; i++) {
                const cloud = new Wolke('white', new L12_Ententeich.Vector(1 + Math.random() * 3, 0), new L12_Ententeich.Vector(-200 + i * 250, 100), new L12_Ententeich.Vector(40, 0.5));
    ...
    class Ente extends Movable {
        ...
        update() {
            ...
            if (this.target) {
                ...
                // Nach dem nächsten Brotkrümel suchen
                if (breadCrumbs.length > 0) {
                    this.target = breadCrumbs[0].position;
                }
                else {
    ...
    function randomPosition() {
        ...
        return new L12_Ententeich.Vector(x, y);
    }
